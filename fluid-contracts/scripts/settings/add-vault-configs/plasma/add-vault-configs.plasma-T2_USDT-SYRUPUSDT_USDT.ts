...
const SUPPLY_TOKEN = TOKENS_PLASMA.DEX_USDT_SYRUPUSDT.address;
const SUPPLY_DECIMALS = TOKENS_PLASMA.DEX_USDT_SYRUPUSDT.decimals; // count of decimals
...
const ORACLE_CONFIG: DexSmartColPegOracleConfig = {
  oracleName: "DexSmartColPegOracle_USDT-SYRUPUSDT_USDT",
  contractName: "DexSmartColPegOracle",
  params: {
    dexPool: SUPPLY_TOKEN,
    quoteInToken0: true, // quote in USDT
    targetDecimals: 15,
    infoName: "USDT per 1 USDT/SYRUPUSDT colSh.",
    pegBufferPercent: 5000, // 10000 = 1%; 100 = 0.01%
    reservesConversionParams: {
      reservesConversionOracle: ORACLES_PLASMA.SYRUPUSDT_USDT_CAPPED,
...
const VAULT_CORE_SETTINGS: VaultT2CoreSettings = {
  ...
  collateralFactor: 88 * PERCENT_PRECISION,
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(3_750_000), // = ~7.5M USD
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  baseDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(7_500_000), // 7.5M USD
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(12_000_000), //12M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
