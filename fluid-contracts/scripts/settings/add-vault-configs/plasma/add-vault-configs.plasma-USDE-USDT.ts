...
const SUPPLY_TOKEN = TOKENS_PLASMA.USDE.address;
const SUPPLY_DECIMALS = TOKENS_PLASMA.USDE.decimals; // count of decimals
...
const VAULT_CORE_SETTINGS: VaultCoreSettings = {
  ...
  collateralFactor: 90 * PERCENT_PRECISION,
  liquidationThreshold: 93 * PERCENT_PRECISION,
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(7_500_000), // = ~7.5M USD
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  ...
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(15_000_000), // 15M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
