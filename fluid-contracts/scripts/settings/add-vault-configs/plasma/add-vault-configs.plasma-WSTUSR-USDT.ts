...
const SUPPLY_TOKEN = TOKENS_PLASMA.WSTUSR.address;
const SUPPLY_DECIMALS = TOKENS_PLASMA.WSTUSR.decimals; // count of decimals
...
const ORACLE_CONFIG: GenericOracleConfig = {
  oracleName: "GenericOracle_WSTUSR_USDT",
  contractName: "FluidGenericOracle",
  infoName: "USDT per 1 WSTUSR",
  targetDecimals: 15,
  sources: [
    {
      sourceType: GenericOracleSourceType.Fluid,
      source: ORACLES_PLASMA.WSTUSR_USR_CAPPED, // FluidCappedRate WSTUSR
      invertRate: false,
      multiplier: 1,
      divisor: 1e12,
    },
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(6_750_000), // = ~7.5M USD
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  ...
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(25_000_000), // 25M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
