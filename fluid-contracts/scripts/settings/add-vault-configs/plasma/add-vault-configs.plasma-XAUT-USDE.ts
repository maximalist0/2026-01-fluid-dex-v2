...
const SUPPLY_TOKEN = TOKENS_PLASMA.XAUT.address;
const SUPPLY_DECIMALS = TOKENS_PLASMA.XAUT.decimals; // count of decimals
...
const BORROW_TOKEN = TOKENS_PLASMA.USDE.address;
const BORROW_DECIMALS = TOKENS_PLASMA.USDE.decimals; // count of decimals
...
const ORACLE_CONFIG: GenericOracleConfig = {
  oracleName: "GenericOracle_XAUT_USDE",
  contractName: "FluidGenericOracle",
  infoName: "USDE per 1 XAUT",
  targetDecimals: 39,
  sources: [
    {
      sourceType: GenericOracleSourceType.Chainlink,
      source: ORACLES_PLASMA.XAU_USD,
      invertRate: false,
      multiplier: BigNumber.from(10).pow(31), // scale XAUT to e27 (+21), adjust USD to USDE (+10)
      divisor: 1,
    },
  ],
};
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(2_070), // = ~7.5M USD
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  baseDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(4_000_000), // 5M USD
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(4_000_000), // 5M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
