...
const SUPPLY_TOKEN = TOKENS_PLASMA.DEX_WETH_WEETH.address;
const SUPPLY_DECIMALS = TOKENS_PLASMA.DEX_WETH_WEETH.decimals; // count of decimals
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(825), // = ~7.5M USD
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  ...
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(1_770), // ~8M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
