...
const ORACLE_CONFIG: GenericOracleConfig = {
  oracleName: "GenericOracle_WEETH_USDE",
  contractName: "FluidGenericOracle",
  infoName: "USDE per 1 WEETH",
  targetDecimals: 27,
  sources: [
    {
      sourceType: GenericOracleSourceType.Chainlink,
      source: ORACLES_PLASMA.WEETH_USD,
      invertRate: false,
      multiplier: BigNumber.from(10).pow(19), // scale WETH to e27 (+9), adjust USD to USDE (+10)
      divisor: 1,
    },
  ],
};
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  baseDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(4_000_000), // 4M USD
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(4_000_000), // 4M USD
  expandDuration: 60 * 60 * 6, // 6hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
