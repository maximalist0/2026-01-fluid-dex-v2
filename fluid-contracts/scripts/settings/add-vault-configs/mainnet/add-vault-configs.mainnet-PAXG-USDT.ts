...
const VAULT_QUEUE_COUNTER = 4; // id 120

const VAULTTYPE = VAULT_TYPE.T1;

const SUPPLY_TOKEN = TOKENS_MAINNET.PAXG.address;
const BORROW_TOKEN = TOKENS_MAINNET.USDT.address;

const ORACLE_CONFIG: GenericOracleConfig = {
  oracleName: "GenericOracle_PAXG_USDT",
  contractName: "FluidGenericOracle",
  infoName: "USDT per 1 PAXG",
  targetDecimals: 15, // scale PAXG 18 to 27 + USDT
  sources: [
    {
      sourceType: GenericOracleSourceType.Chainlink,
      source: "0x214eD9Da11D2fbe465a6fc601a91E62EbEc1a0D6", // XAU<>USD
      invertRate: false,
      multiplier: BigNumber.from(10).pow(9), // scale PAXG to e27 (+9), adjust USD to USDT (-2)
      divisor: 1e2,
    },
