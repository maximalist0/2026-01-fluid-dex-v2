...
const VAULT_CORE_SETTINGS: VaultCoreSettings = {
  ...
  liquidationThreshold: 86 * PERCENT_PRECISION,
  liquidationMaxLimit: (925 * PERCENT_PRECISION) / 10, // 92.5%
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(2300), // 2300 WSTETH = ~7.5M USDT at 1 WSTETH = 3266 USDT
...
const VAULT_WITHDRAW_LIMIT_CHECKPOINTS: ExpandPercentConfigHandlerStructs.LimitCheckPointsStruct = {
  ...
  tvlCheckPoint1: SUPPLY_DECIMALS_MULTIPLIER.mul(6122), // 6122 WSTETH = ~20M USDT at 1 WSTETH = 3266 USDT
  expandPercentUntilCheckPoint1: 25 * PERCENT_PRECISION, // e.g. 25%
  tvlCheckPoint2: SUPPLY_DECIMALS_MULTIPLIER.mul(9184), // 9184 WSTETH = ~30M USDT at 1 WSTETH = 3266 USDT
  expandPercentUntilCheckPoint2: 20 * PERCENT_PRECISION, // e.g. 20%
  tvlCheckPoint3: SUPPLY_DECIMALS_MULTIPLIER.mul(12245), // 12245 WSTETH = ~40M USDT at 1 WSTETH = 3266 USDT
...
const ORACLE_CONFIG: WstETHCLRS2UniV3CheckCLRSOracleConfig = {
  oracleName: "WstETHCLRS2UniV3CheckCLRSOracle_WSTETH_USDT",
  contractName: "WstETHCLRS2UniV3CheckCLRSOracle",
  wstETHCLRS2Params: {
    wstETH: TOKENS_MAINNET.WSTETH.address,
    // wstETH & CL 1 hop Oracle:
    // 1. hop: WSTETH <> STETH (Get amount of stETH for a one wstETH)
    // 2. hop: Chainlink STETH <> ETH
    chainlinkParams: {
      hops: 1,
      feed1: {
