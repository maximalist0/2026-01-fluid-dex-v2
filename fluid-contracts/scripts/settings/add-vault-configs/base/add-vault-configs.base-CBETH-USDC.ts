...
const SUPPLY_TOKEN = TOKENS_BASE.CBETH.address;
const SUPPLY_DECIMALS = TOKENS_BASE.CBETH.decimals; // count of decimals
...
const BORROW_TOKEN = TOKENS_BASE.USDC.address; // USDC
const BORROW_DECIMALS = TOKENS_BASE.USDC.decimals; // count of decimals
...
const VAULT_CORE_SETTINGS: VaultCoreSettings = {
  ...
  collateralFactor: 70 * PERCENT_PRECISION,
  liquidationThreshold: 76 * PERCENT_PRECISION,
  liquidationMaxLimit: 82 * PERCENT_PRECISION,
  withdrawGap: 5 * PERCENT_PRECISION,
  liquidationPenalty: 6 * PERCENT_PRECISION,
  borrowFee: 0,
};
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(2_030), // 2_030 CBETH = ~7.5M USD @ 1CBETH = 3700
...
const VAULT_BORROW_CONFIG = (vault: string): AdminModuleStructs.UserBorrowConfigStruct => ({
  baseDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(7_500_000), // 7.5M USDC
  maxDebtCeiling: BORROW_DECIMALS_MULTIPLIER.mul(200_000_000), // 200M USDC
  expandDuration: 60 * 60 * 12, // 12hrs in seconds.
  expandPercent: 20 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
...
const ORACLE_CONFIG: FallbackCLRSOracleConfig = {
  oracleName: "FallbackCLRSOracleL2_CBETH_USDC",
  contractName: "FallbackCLRSOracleL2",
  infoName: "USDC for 1 CBETH",
  chainlinkParams: {
    hops: 2,
    feed1: {
      feed: "0xd7818272B9e248357d13057AAb0B417aF31E817d", // CBETH / USD
      invertRate: false,
      token0Decimals: 18,
    } as ChainlinkStructs.ChainlinkFeedDataStruct,
    feed2: {
      feed: "0x7e860098F58bBFC8648a4311b374B1D669a2bc6B", // USDC / USD
      invertRate: true,
      token0Decimals: 6,
    } as ChainlinkStructs.ChainlinkFeedDataStruct,
    feed3: {
