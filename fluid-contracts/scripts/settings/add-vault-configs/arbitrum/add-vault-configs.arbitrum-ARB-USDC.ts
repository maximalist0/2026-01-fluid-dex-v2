...
const SUPPLY_TOKEN = TOKENS_ARBITRUM.ARB.address;
const SUPPLY_DECIMALS = TOKENS_ARBITRUM.ARB.decimals; // count of decimals
...
const BORROW_TOKEN = TOKENS_ARBITRUM.USDC.address; // USDC
const BORROW_DECIMALS = TOKENS_ARBITRUM.USDC.decimals; // count of decimals
...
const VAULT_CORE_SETTINGS: VaultCoreSettings = {
  ...
  liquidationPenalty: 10 * PERCENT_PRECISION,
  borrowFee: 0,
};
...
const VAULT_SUPPLY_CONFIG = (vault: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: SUPPLY_DECIMALS_MULTIPLIER.mul(12_500_000), // 12_500_000 ARB = ~7.5M USD @ 1ARB = 0.60
...
const ORACLE_CONFIG: FallbackCLRSOracleConfig = {
  oracleName: "FallbackCLRSOracleL2_ARB_USDC",
  contractName: "FallbackCLRSOracleL2",
  infoName: "USDC for 1 ARB",
  chainlinkParams: {
    hops: 2,
    feed1: {
