...
const TOKEN = TOKENS_ARBITRUM.SUSDS.address;
const DECIMALS = TOKENS_ARBITRUM.SUSDS.decimals; // count of decimals
...
const LIQUIDITY_RATE_DATA: AdminModuleStructs.RateDataV1ParamsStruct | AdminModuleStructs.RateDataV2ParamsStruct = {
  kink: 90 * PERCENT_PRECISION,
  rateAtUtilizationZero: 0 * PERCENT_PRECISION,
  rateAtUtilizationKink: 0 * PERCENT_PRECISION,
  rateAtUtilizationMax: 10 * PERCENT_PRECISION,
  token: TOKEN,
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(7_500_000), // 7.5M
