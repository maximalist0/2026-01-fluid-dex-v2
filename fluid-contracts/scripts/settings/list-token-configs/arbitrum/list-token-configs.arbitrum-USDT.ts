...
import { AVOCADO_TEAM_MULTISIG } from "../contract-addresses";
...
const TOKEN = TOKENS_ARBITRUM.USDT.address;
const DECIMALS = TOKENS_ARBITRUM.USDT.decimals; // count of decimals
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(7_500_000), // 7.5M USDT
...
const LENDING_REWARDS_RATE_MODEL = {
  ...
  startTvl: DECIMALS_MULTIPLIER.mul(1_000), // start at 1000 USDT
  rewardAmount: DECIMALS_MULTIPLIER.mul(300_000), // 3 x 100_000 USDT for total 90 days
  initiator: "0x4F6F977aCDD1177DCD81aB83074855EcB9C2D49e",
  previousModel: constants.AddressZero,
};

// set to null to skip ARB StakingRewards (rewards in ARB):
...
const STAKING_REWARDS = (fTokenAddress: string) => ({
  ...
  //
  rewardsToken: TOKENS_ARBITRUM.ARB.address, // ARB. SHOULD NOT CHANGE
