...
const TOKEN = TOKENS_POLYGON.WSTETH.address;
const DECIMALS = TOKENS_POLYGON.WSTETH.decimals; // count of decimals
...
const LIQUIDITY_RATE_DATA: AdminModuleStructs.RateDataV1ParamsStruct | AdminModuleStructs.RateDataV2ParamsStruct = {
  kink: 70 * PERCENT_PRECISION,
  rateAtUtilizationZero: 0 * PERCENT_PRECISION,
  rateAtUtilizationKink: Math.round(1.1 * PERCENT_PRECISION),
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(2_750), // = ~7.5M USD @ 1WSTETH = 2727
