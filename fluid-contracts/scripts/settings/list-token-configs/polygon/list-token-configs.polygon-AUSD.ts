...
const TOKEN = TOKENS_POLYGON.AUSD.address;
const DECIMALS = TOKENS_POLYGON.AUSD.decimals; // count of decimals
...
const LIQUIDITY_RATE_DATA: AdminModuleStructs.RateDataV1ParamsStruct | AdminModuleStructs.RateDataV2ParamsStruct = {
  ...
  rateAtUtilizationKink1: Math.round(4.1 * PERCENT_PRECISION),
  rateAtUtilizationKink2: Math.round(5.6 * PERCENT_PRECISION),
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(7_500_000), // 7.5M AUSD
