...
const TOKEN = TOKENS_POLYGON.WETH.address;
const DECIMALS = TOKENS_POLYGON.WETH.decimals; // count of decimals
...
const LIQUIDITY_RATE_DATA: AdminModuleStructs.RateDataV1ParamsStruct | AdminModuleStructs.RateDataV2ParamsStruct = {
  kink1: 75 * PERCENT_PRECISION,
  kink2: 85 * PERCENT_PRECISION,
  rateAtUtilizationZero: 0 * PERCENT_PRECISION,
  rateAtUtilizationKink1: Math.round(2.3 * PERCENT_PRECISION),
  rateAtUtilizationKink2: 3 * PERCENT_PRECISION,
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(3_285), // = ~7.5M USD @ 1ETH = 2283
