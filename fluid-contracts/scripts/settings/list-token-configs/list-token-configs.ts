...
const LIQUIDITY_RATE_DATA: AdminModuleStructs.RateDataV1ParamsStruct | AdminModuleStructs.RateDataV2ParamsStruct = {
  kink: 60 * PERCENT_PRECISION,
  rateAtUtilizationZero: 5 * PERCENT_PRECISION,
  rateAtUtilizationKink: 10 * PERCENT_PRECISION,
  rateAtUtilizationMax: 80 * PERCENT_PRECISION,
  token: TOKEN,
...
const LIQUIDITY_TOKEN_CONFIG: AdminModuleStructs.TokenConfigStruct = {
  ...
  threshold: 1 * PERCENT_PRECISION, // update storage at >1% changes in utilization
  token: TOKEN,
...
// BELOW CONFIG FOR FTOKEN NOT NEEDED FOR LISTING NATIVE TOKEN (is part of wrapped native token listing instead).
...
const FTOKEN_SUPPLY_CONFIG = (fTokenAddress: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  ...
  baseWithdrawalLimit: DECIMALS_MULTIPLIER.mul(1e5),
  expandDuration: 1, // in seconds
  expandPercent: 0 * PERCENT_PRECISION, // in 1e2
...
const FTOKEN_WITHDRAW_LIMIT_CHECKPOINTS: ExpandPercentConfigHandlerStructs.LimitCheckPointsStruct = {
  ...
  tvlCheckPoint1: DECIMALS_MULTIPLIER.mul(0), // e.g. 20M
  expandPercentUntilCheckPoint1: 0 * PERCENT_PRECISION, // e.g. 25%
  tvlCheckPoint2: DECIMALS_MULTIPLIER.mul(0), // e.g. 30M
  expandPercentUntilCheckPoint2: 0 * PERCENT_PRECISION, // e.g. 20%
  tvlCheckPoint3: DECIMALS_MULTIPLIER.mul(0), // e.g. 40M
  expandPercentUntilCheckPoint3: 0 * PERCENT_PRECISION, // e.g. 15%
  expandPercentAboveCheckPoint3: 0 * PERCENT_PRECISION, // e.g. 10%
};

// set to null to skip LendingRewardsRateModel (rewards in assets):
...
const STAKING_REWARDS = (fTokenAddress: string) => ({
  ...
  //
  rewardsToken: TOKENS_MAINNET.INST.address, // SHOULD NOT CHANGE
