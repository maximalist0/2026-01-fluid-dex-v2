...
const DEX_QUEUE_COUNTER = 0; // id 11

// token0 has to be smaller than token1
const TOKEN0 = TOKENS_PLASMA.GHO;
const TOKEN1 = TOKENS_PLASMA.SYRUPUSDT;
...
const DEX_SUPPLY_TOKEN0_CONFIG = (dexT1: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: TOKEN0_DECIMALS_MULTIPLIER.mul(7_000),
...
const DEX_SUPPLY_TOKEN1_CONFIG = (dexT1: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: TOKEN1_DECIMALS_MULTIPLIER.mul(6_000),
  expandDuration: 60 * 60 * 1, // 1hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
...
const DEX_INIT_SETTINGS: DexAdminModuleStructs.InitializeVariablesStruct = {
  smartCol: true,
  token0ColAmt: TOKEN0_DECIMALS_MULTIPLIER.mul(8),
  smartDebt: false,
  token0DebtAmt: 0,
  centerPrice: "909354595245663240229345579",
  fee: 1e4 * 0.02,
  revenueCut: 1e4 * 25,
  upperPercent: 1e4 * 0.35,
  lowerPercent: 1e4 * 0.35,
  upperShiftThreshold: 0,
  lowerShiftThreshold: 0,
  thresholdShiftTime: DEXT1_THRESHOLD_SHIFT_TIME_MAX,
  centerPriceAddress: 23,
  hookAddress: 0,
  maxCenterPrice: BigNumber.from(10).pow(27).mul(1),
  minCenterPrice: BigNumber.from(10).pow(27).div(100),
};

const DEX_MAX_SUPPLY_SHARES = BigNumber.from(10).pow(18).mul(5_000);
