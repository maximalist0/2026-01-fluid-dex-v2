...
const TOKEN0 = TOKENS_PLASMA.WSTUSR;
...
const DEX_SUPPLY_TOKEN0_CONFIG = (dexT1: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: TOKEN0_DECIMALS_MULTIPLIER.mul(4_500_000), // = ~5M USD
...
const DEX_SUPPLY_TOKEN1_CONFIG = (dexT1: string): AdminModuleStructs.UserSupplyConfigStruct => ({
  baseWithdrawalLimit: TOKEN1_DECIMALS_MULTIPLIER.mul(5_000_000), // = ~5M USD
  expandDuration: 60 * 60 * 1, // 1hrs in seconds.
  expandPercent: 50 * PERCENT_PRECISION, // in 1e2
  mode: MODE_WITH_INTEREST, // with interest.
  //
...
const DEX_INIT_SETTINGS: DexAdminModuleStructs.InitializeVariablesStruct = {
  smartCol: true,
  token0ColAmt: TOKEN0_DECIMALS_MULTIPLIER.mul(4), // ~5 USD
  smartDebt: false,
  token0DebtAmt: 0,
  centerPrice: "1110230734857430355000000000",
  fee: 1e4 * 0.02,
  revenueCut: 25 * 1e4,
  upperPercent: 1e4 * 0.25,
  lowerPercent: 1e4 * 0.25,
  upperShiftThreshold: 0,
  lowerShiftThreshold: 0,
  thresholdShiftTime: DEXT1_THRESHOLD_SHIFT_TIME_MAX,
  centerPriceAddress: 5,
  hookAddress: 0,
  maxCenterPrice: utils.parseUnits("100", 27),
  minCenterPrice: utils.parseUnits("0.11", 27),
};

const DEX_MAX_SUPPLY_SHARES = BigNumber.from(10).pow(18).mul(6_000_000); // ~12M USD
