...
import {
  ...
  configCoreReserve,
  configCoreVault,
  configCoreSmartLending,
} from "../config";
import { FluidVersion } from "../settings";
import { logDebug, logSuccess, TxQueue } from "../util";

export const prodInitialConfigs = async (hre: HardhatRuntimeEnvironment) => {
  logDebug("\n\n------------------- FLUID INITIAL CORE CONFIGS -------------------\n");
  ...
  // await configCoreSmartLending(hre, version);

  await TxQueue.processQueue(hre, "core-configs");

  logDebug("\n-----------------------------------------");
  logSuccess(
    chalk.bold.underline("Executed all steps for Fluid", version.replace(/_/g, "."), "initial core configs!\n")
  );
  console.log(
