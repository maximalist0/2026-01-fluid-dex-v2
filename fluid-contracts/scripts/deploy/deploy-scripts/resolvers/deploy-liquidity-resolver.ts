...
export const deployLiquidityResolver = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
...
const deployV1: DeployFunction = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
  ...
  const deployedAddress = await deploy(
    hre,
    "LiquidityResolver",
    "contracts/periphery/resolvers/liquidity/main.sol:FluidLiquidityResolver",
    version,
    [liquidity.address]
  );
  return deployedAddress;
