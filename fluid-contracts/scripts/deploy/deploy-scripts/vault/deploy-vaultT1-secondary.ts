...
export const deployVaultT1Secondary = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
...
const deployV1: DeployFunction = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
  const deployedAddress = await deploy(
    hre,
    "VaultT1Secondary",
    "contracts/protocols/vault/vaultT1/coreModule/main2.sol:FluidVaultT1Secondary",
    version,
    []
  );
  return deployedAddress;
