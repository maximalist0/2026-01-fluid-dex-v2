...
export const deployDexFeeAuth = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
...
const deployV1: DeployFunction = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
  const name = "DexFeeAuth";

  const deployedAddress = await deploy(hre, name, "contracts/config/dexFeeAuth/main.sol:FluidDexFeeAuth", version, []);
  return deployedAddress;
};
