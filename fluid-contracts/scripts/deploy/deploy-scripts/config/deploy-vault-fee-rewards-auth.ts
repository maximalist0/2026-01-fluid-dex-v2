...
export const deployVaultFeeRewardsAuth = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
...
const deployV1: DeployFunction = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
  const name = "VaultFeeRewardsAuth";

  const deployedAddress = await deploy(
    hre,
    name,
    "contracts/config/vaultFeeRewardsAuth/main.sol:FluidVaultFeeRewardsAuth",
    version,
    []
  );
  return deployedAddress;
