...
export const deployLiquidityZircuitTransferModuleMainnet = async (
...
const deployV1: DeployFunction = async (hre: HardhatRuntimeEnvironment, version: FluidVersion) => {
  const deployedAddress = await deploy(
    ...
    "contracts/liquidity/zircuitTransferModuleMainnet/main.sol:FluidLiquidityZircuitTransferModuleMainnet",
    version,
    []
  );
  return deployedAddress;
