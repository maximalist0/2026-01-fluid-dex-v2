...
There are tests for mainnet version of contracts and for non-mainnet version of the Liquidity Layer. By default, `forge test` runs the mainnet version.
To run the non-mainnet ("others") version, execute `RUN_OTHERS=true forge test -v --ffi`
